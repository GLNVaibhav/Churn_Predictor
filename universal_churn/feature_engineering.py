...
UNIVERSAL_MODEL_PATH, UNIVERSAL_NORM_STATS_PATH,
UNIVERSAL_FEATURES_PATH,
)
from .preprocessing import sanitize_numerical_columns


def _norm_max(df: pd.DataFrame, col: str | None, sector: str,
              norm_stats: dict | None) -> float:
    """
    Return the normalization maximum for `col` in `sector`.
    Uses persisted training-set statistics when available.
    """
    if col is None:
        return 1.0
    key = f"{sector}.{col}"
...
def compute_norm_stats(df: pd.DataFrame, sector: str,
                       columns: list[str]) -> dict:
    """Compute per-column maxima for one sector."""
    return {
...
def extract_universal_features(
    df: pd.DataFrame, sector: str, target_col: str,
    ...
    if sector == 'telecom':
        ...
        _has_visits = any(c in df.columns for c in ('Visits_Last_Year', 'visitslastyear'))
        ...
        if _has_tenure or _has_visits:
            ...
            c_cost = _hcol(df, 'Avg_Out_Of_Pocket_Cost', 'avgoutofpocketcost')
            ...
            feat['tenure_normalized'] = df[c_tenure] / max_tenure if c_tenure else pd.Series(0.5, index=df.index)
            feat['charge_normalized'] = df[c_cost] / max_cost if c_cost else pd.Series(0.0, index=df.index)
            feat['has_complaint'] = (df[c_billing] > 0).astype(int) if c_billing else pd.Series(0, index=df.index)
            feat['satisfaction_score'] = df[c_sat] / max_sat if c_sat else pd.Series(0.5, index=df.index)
            feat['is_active'] = (df[c_lastv] <= 90).astype(int) if c_lastv else pd.Series(0.5, index=df.index)
            feat['num_products_services'] = df[c_visits] / max_visits if c_visits else pd.Series(0.5, index=df.index)
            feat['is_senior_or_high_risk'] = (df[c_age] > 65).astype(int) if c_age else pd.Series(0, index=df.index)
            feat['has_support'] = df[c_portal] if c_portal else pd.Series(0, index=df.index)
            feat['contract_stability'] = df[c_tenure] / max_tenure if c_tenure else pd.Series(0.5, index=df.index)
            feat['payment_auto'] = 0.5
            feat['engagement_score'] = feat['num_products_services']
            feat['coupon_dependency'] = 0
            feat['cashback_engagement'] = 0
            feat['recency_score'] = 1 - (df[c_lastv] / max_lastv) if c_lastv else pd.Series(0.5, index=df.index)
            feat['convenience_score'] = 1 - (df[c_dist] / max_dist) if c_dist else pd.Series(0.5, index=df.index)
            feat['dormant_loyalty_risk'] = feat['tenure_normalized'] * (1 - feat['is_active'])
            feat['lockin_risk'] = 0
            ...
            # FIX: Wrap scalar fallbacks in pd.Series to support .clip()
            ...
            feat['referral_engagement'] = (df[c_refs] / max_refs).clip(0, 1) if c_refs else pd.Series(0.0, index=df.index)
            ...
            feat['missed_appt_rate'] = 0
            feat['composite_satisfaction'] = feat['satisfaction_score']
            feat['billing_friction'] = (df[c_calls] / max_calls).clip(0, 1) if c_calls else pd.Series(0.0, index=df.index)
            feat['care_accessibility'] = 0.5
            feat['referral_engagement'] = 0
        else:
    ...
    if target_col_actual in df.columns:
        y = df[target_col_actual].astype(str).str.strip()
